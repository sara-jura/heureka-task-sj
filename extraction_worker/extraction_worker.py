...
class ExtractionWorker:
    ...
    def __init__(self, postgres_dao: PostgresDAO) -> None:
        ...
        self.logger = setup_logging(
            log_level=logging.INFO, logger_name="offer_extraction_worker"
        )
    ...
    def validate_offer_data(self, body: Dict) -> bool:
        ...
        for field in LEGACY_FIELDS:
            if field not in body["legacy"]:
                self.logger.warning(
                    f"Offer legacy data missing the key '{field}' ({body}"
                )
                return False
        ...
        try:
            ...
            self.logger.warning(
                f"Offer id not in the proper UUID format ('{body['id']}')"
            )
            return False
        return True
    ...
    def process_offer(self, ch, method, properties, body) -> None:
        ...
        try:
            ...
            if self.validate_offer_data(body):
                # if data passes validation save the offer data into the target table,
                # if an offer with the same id exists update the parameters
                query = f"""
            ...
            self.logger.warning(
                f"Something went wrong when processing a message: {str(e)}"
            )
    ...
    def process_messages(self) -> None:
        ...
        try:
            ...
            self.channel.basic_consume(
                queue=QUEUE_NAME, on_message_callback=self.process_offer, auto_ack=True
            )
...
def main():
    ...
    postgres_dao = PostgresDAO(
        db=db_name, username=db_user, password=db_pass, port=db_port, host=db_host
    )
